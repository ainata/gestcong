...
$mysqlQuery2 = @mysql_query($sql2, $mysqlCnx) or die('Pb de requête'.mysql_error());
...
$sql = " SELECT * from employer where ID_DEPARTEMENT=".$_SESSION['departement']." order by PRENOM ";
$mysqlQuery = @mysql_query($sql, $mysqlCnx) or die('Pb de requête 1'.mysql_error());

$l=0;
while ($row = mysql_fetch_array($mysqlQuery,  MYSQL_ASSOC)) {
	
	$tableauNB_jr[$l] = $row['SOLDE_PERMISSION'];
	//echo $tableauEmployer2[$y];
	$l=$l+1;
...
// Le titre
$graph->title->Set("*** GRAPHE SOLDE PERMISSION RESTANT ***");
