...
$sql3 = " SELECT ID_EMPLOYER FROM employer order by PRENOM ";
...
$sql1 = " SELECT deposition.ID_EMPLOYER as ID_EMPLOYER_DEPOSITION,MONTH(DATE_DEPART) as MOIS,YEAR(DATE_DEPART) as YEAR,DATE_DEPART,DATE_RETOUR,ID_DEPOSITION,JOURNE_DE_DEPART,JOURNE_DE_RETOUR FROM deposition where YEAR(DATE_DEPART)='".$row_year_2."' and ID_EMPLOYER='".$tableauEmployer31[$Z]."' and ID_DEPOSITION IN (SELECT ID_DEPOSITION from valider WHERE VALIDE_DEPARTEMENT='oui' and VALIDE_RESPONSABLE='oui') order by ID_EMPLOYER_DEPOSITION ";
...
// Le titre
$graph->title->Set("*** GRAPHE DE TOUS LES CONGES PRIS PAR LES EMPLOYES PAR DEPARTEMENT AU COURS D'ANNEE ".$row_year_1." ***");
...
// Le titre
$graph1->title->Set("*** GRAPHE DE TOUS LES CONGES PRIS PAR LES EMPLOYES PAR DEPARTEMENT AU COURS D'ANNEE ".$row_year_2." ***");
